...
class TiliTripApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.status_btn = ctk.CTkButton(self.scroll_canvas, text="🔄 СМЕНИТЬ СТАТУС",
                                        fg_color=COLORS["accent"],
                                        command=self.change_trip_status)
        self.status_btn.pack(side="left", padx=20)
        ctk.CTkButton(self.scroll_canvas, text="УДАЛИТЬ ПОЕЗДКУ", fg_color=COLORS["danger"],
                      command=self.delete_trip).pack(anchor="e", padx=20)
        ...
        # Таблица плана
        self.plan_view = ttk.Treeview(self.scroll_canvas, columns=("LocID", "Status", "Day", "City", "Cost"),
        ...
        # Итоговая сумма
        self.total_cost_label = ctk.CTkLabel(self.scroll_canvas, text="ИТОГО ПОЕЗДКА: 0 ₽",
    ...
    def change_trip_status(self):
        ...
        # Логика переключения: Планируется -> В процессе -> Завершено -> Планируется
        try:
        ...
        # Обновляем список, чтобы увидеть изменения (и цвета, если они настроены)
        self.update_trip_list()

        # Сохраняем выделение на той же строке
