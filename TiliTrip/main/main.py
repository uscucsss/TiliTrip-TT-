...
class TiliTripApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.status_menu.grid(row=0, column=4, padx=10)
        self.status_menu.set("Планируется")

        ctk.CTkButton(self.frame_add, text="СОЗДАТЬ", command=self.add_trip, width=100).grid(row=0, column=5, padx=15)
    ...
    def _route_thread(self, trip_data):
        try:
            ...
            self.after(0, lambda: messagebox.showerror("Ошибка", f"Расчет не удался: {e}"))

    def open_map_action(self):
        sel = self.tree.selection()
        if not sel: return
        trip_id, _, _, dep_city, _ = self.tree.item(sel)['values']
        threading.Thread(target=self._map_thread, args=(trip_id, dep_city), daemon=True).start()
    ...
    def add_location(self):
        sel = self.tree.selection()
        if not sel: return
        t_id, city, day = self.tree.item(sel)['values'][0], self.entry_city.get(), self.entry_day.get()
        if city and day:
