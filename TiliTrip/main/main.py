...
class TiliTripApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.date_picker.grid(row=0, column=2, padx=5, pady=25)
        ...
        self.entry_city = ctk.CTkEntry(self.frame_loc_add, placeholder_text="Куда идем?")
    ...
    def get_coords(self, city, loc_id=None):
        ...
        try:
            ...
            headers = {'User-Agent': 'TiliTripPlanner_App_v2.5_Stable'}
    ...
    def calculate_route(self):
        ...
        trip_data = self.tree.item(sel[0])['values']
        self.result_label.configure(text="⏳ Считаю маршрут...")
    ...
    def _route_thread(self, trip_data):
        try:
            ...
            if l_start: full_pts.append(f"{l_start},{lon_start}")
            ...
            for l_id, l_city in locs:
                ...
                if lt: full_pts.append(f"{lt},{ln}")
                else: return self.after(0, lambda c=l_city: messagebox.showerror("Ошибка", f"Не найден город: {c}"))

            if len(full_pts) < 2: return self.after(0, lambda: self.result_label.configure(text="Нужно минимум 2 точки"))

            dist, dur = 0, 0
            api_key = "e0db81e2-ded2-4ba9-9f0f-dc8eb9fac721"
            for i in range(len(full_pts)-1):
                url = "https://graphhopper.com"
                p = [('point', full_pts[i]), ('point', full_pts[i+1]), ('profile', 'car'), ('key', api_key)]
                r = requests.get(url, params=p).json()
                if 'paths' in r:
                    dist += r['paths'][0]['distance']
                    dur += r['paths'][0]['time']
                else: raise Exception(r.get('message', 'Ошибка API'))
    ...
    def open_map_action(self):
        sel = self.tree.selection()
        if not sel: return
        t_id, _, _, dep, _ = self.tree.item(sel[0])['values']
    ...
    def add_trip(self):
        name, dep, date, status = self.entry_trip_name.get(), self.entry_dep_city.get(), self.date_picker.get(), self.status_menu.get()
        if name and dep:
    ...
    def on_trip_select(self):
        sel = self.tree.selection()
        if sel: self.update_plan_view(self.tree.item(sel[0])['values'][0])
    ...
    def update_plan_view(self, trip_id):
        ...
        for r in self.cursor.fetchall():
            self.plan_view.insert("", "end", values=(r[0], "✅ Готово" if r[1] else "⏳ План", r[2], r[3]), tags=('item_done' if r[1] else ''))

    def add_location(self):
        sel = self.tree.selection()
        if not sel: return
        t_id, city, day = self.tree.item(sel[0])['values'][0], self.entry_city.get(), self.entry_day.get()
        if city and day:
            self.cursor.execute("INSERT INTO locations (trip_id, city, day_number) VALUES (?,?,?)", (t_id, city, day))
            self.conn.commit(); self.update_plan_view(t_id)

    def delete_location(self):
        sel = self.plan_view.selection()
        if sel:
            self.cursor.execute("DELETE FROM locations WHERE id=?", (self.plan_view.item(sel[0])['values'][0],))
            self.conn.commit(); self.on_trip_select()

    def toggle_location_done(self):
        sel = self.plan_view.selection()
        if sel:
            self.cursor.execute("UPDATE locations SET is_done = NOT is_done WHERE id=?", (self.plan_view.item(sel[0])['values'][0],))
            self.conn.commit(); self.on_trip_select()

    def delete_trip(self):
        sel = self.tree.selection()
        if sel and messagebox.askyesno("Удаление", "Удалить поездку?"):
            self.cursor.execute("DELETE FROM trips WHERE id=?", (self.tree.item(sel[0])['values'][0],))
            self.conn.commit(); self.update_trip_list()

    def clear_entire_plan(self):
        sel = self.tree.selection()
        if sel:
            self.cursor.execute("DELETE FROM locations WHERE trip_id=?", (self.tree.item(sel[0])['values'][0],))
            self.conn.commit(); self.on_trip_select()
